...
def train_net(net):
    ...
    for epoch in range(n_epochs):
        ...
        for index in range(n_batches_train):
            ...
            measures_sum = [measure_sum + measure for measure_sum,
                            measure in zip(measures_sum, measures)]
            measures_avg = [measure_sum / (index+1)
                            for measure_sum in measures_sum]
            ...
            experiment.log_metric('accuracy', 100 - measures_avg[2], step=_step)
            ...
            Delta_logW = net.weakly_clamped_phase(
                n_it_pos, epsilon, beta, *alphas)
            gW = [gW1 + Delta_logW1 for gW1,
                  Delta_logW1 in zip(gW, Delta_logW)]

        stdout.write("\n")
        dlogW = [100. * gW1 / n_batches_train for gW1 in gW]
        print("   "+" ".join(["dlogW%i=%.3f%%" % (k+1, dlogW1)
        ...
        for index in range(n_batches_valid):
            ...
            measures_avg = [measure_sum / (index+1)
                            for measure_sum in measures_sum]
            measures_avg[-1] *= 100.  # measures_avg[-1] corresponds to the error rate, which we want in percentage
            stdout.write("\r   valid-%5i E=%.1f C=%.5f error=%.2f%%" % ((index+1)
                                                                        * batch_size, measures_avg[0], measures_avg[1], measures_avg[2]))
            stdout.flush()
