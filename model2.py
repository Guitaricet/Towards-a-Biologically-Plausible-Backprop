...
class Network(object):
    ...
    def __energy(self, layers):
        squared_norm = sum([T.batched_dot(layer, layer)
                            for layer in layers]) / 2.
        linear_terms = - sum([T.dot(layer, b)
                              for layer, b in zip(layers, self.biases)])
        quadratic_terms = - sum([T.batched_dot(T.dot(pre, W), post)
    ...
    def __build_negative_phase(self):
        ...
        def forwardprop(*layers):
            ...
            for k in range(1, len(self.layers)-1):
                layers_new += [pi(T.dot(layers_new[-1], self.weights[k-1]) +
                                  T.dot(layers[k+1], self.weights[k].T) + self.biases[k])]
            layers_new += [pi(T.dot(layers_new[-1],
    ...
    def __build_positive_phase(self):
        ...
        def backprop(*layers):
            ...
            for k in range(len(self.layers)-2, 0, -1):
                layers_new += [pi(T.dot(layers[k-1], self.weights[k-1]) +
                                  T.dot(layers_new[-1], self.weights[k].T) + self.biases[k])]
            layers_new += [layers[0]]
            layers_new.reverse()
            return layers_new
        ...
        Delta_layers = [(layer_new-layer) for layer_new,
                        layer in zip(layers_new[1:], self.layers[1:])]
        biases_new = [b + alpha * T.mean(Delta, axis=0)
                      for b, alpha, Delta in zip(self.biases[1:], alphas, Delta_layers)]
        weights_new = [W + alpha * T.dot(layer.T, Delta) / T.cast(self.layers[0].shape[0], dtype=theano.config.floatX)
                       for W, alpha, layer, Delta in zip(self.weights, alphas, self.layers[:-1], Delta_layers)]
